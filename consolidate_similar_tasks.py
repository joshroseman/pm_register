...
for key, tasks in name_groups.items():
    ...
    if all_identical:
        ...
        non_informative = [
            (idx, row) for idx, row in tasks if has_non_informative_description(row)
        ]
        ...
        basic_desc = [
            ...
            if not has_non_informative_description(row)
            and not has_rich_description(row)
        ]
        ...
        for initiative, init_tasks in initiative_groups.items():
            if len(init_tasks) == 1:
                ...
                tasks_to_rename.append(
                    (keep_idx, keep_row["TaskID"], keep_row["TaskName"], new_name)
                )
...
with open(report_path, "w") as f:
    ...
    if tasks_to_rename:
        ...
        for _, task_id, old_name, new_name in tasks_to_rename[
            :20
