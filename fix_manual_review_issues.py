...
def detect_circular_refs(task_id, parent_id, hierarchy_map):
    ...
    for _ in range(5):  # Limit depth to avoid infinite loops
        ...
        for child_id in hierarchy_map[current_id]:
            ...
            if child_id in hierarchy_map and any(ref in visited for ref in hierarchy_map[child_id]):
                return True
...
for parent_id, children in hierarchy_map.items():
    ...
    if not parent_row.empty:
        ...
        for child_id in children:
            ...
            if not child_idx.empty and df.loc[child_idx[0], 'Initiative'] != parent_initiative:
...
print(f"Marked {len(rasa_not_info)} tasks with RASA references as informational contexts")
...
print(f"Current ParentTaskID links: {current_percentage:.2f}% ({current_parent_links} tasks)")
...
for initiative in initiatives:
    ...
    if 'TranscriptionGroup' in df.columns:
        transcription_groups = potential_children['TranscriptionGroup'].dropna().unique()
        
        for group in transcription_groups:
            if added_links >= links_to_add:
                break
                
            group_tasks = potential_children[potential_children['TranscriptionGroup'] == group]
            ...
            if len(group_tasks) >= 2:
                ...
                for idx, child in group_tasks.iloc[1:].iterrows():
                    ...
                    if child_id in hierarchy_map or pd.notna(df.loc[idx, 'ParentTaskID']) and df.loc[idx, 'ParentTaskID'] != '':
                        continue
        ...
        for key, tasks in name_groups.items():
            if len(tasks) >= 2 and added_links < links_to_add:
                ...
                for child_idx, child_row in tasks[1:]:
                    ...
                    if child_id in hierarchy_map or pd.notna(df.loc[child_idx, 'ParentTaskID']) and df.loc[child_idx, 'ParentTaskID'] != '':
                        continue
...
if 'TranscriptionGroup' in df.columns and 'ConversationFlowID' in df.columns:
    ...
    df.loc[empty_flow_mask, 'ConversationFlowID'] = df.loc[empty_flow_mask, 'TranscriptionGroup']
    metadata_updates += empty_flow_mask.sum()
    print(f"Populated {empty_flow_mask.sum()} ConversationFlowID values from TranscriptionGroup")
...
if 'ProcessingStageNotes' in df.columns:
    if 'invalid_parents_mask' in locals():
        df.loc[invalid_parents_mask, 'ProcessingStageNotes'] = 'Fixed invalid parent reference'
    ...
    if circular_refs_fixed > 0:
        df.loc[circular_refs_mask, 'ProcessingStageNotes'] = 'Fixed circular reference in hierarchy'
        metadata_updates += circular_refs_fixed
    
    print(f"Added ProcessingStageNotes for {invalid_parents_mask.sum() + circular_refs_fixed} tasks with fixed issues")

if 'MetadataCompleteness' in df.columns:
    metadata_fields = [
        'TaskName', 'TaskDescription', 'Assignee', 'Initiative', 'Status',
        'ParentTaskID', 'SubTaskLevel', 'ClassificationCategory', 'IsInformationalContext'
    ]
    
    for idx, row in df.iterrows():
        filled_count = sum(1 for field in metadata_fields if field in df.columns and pd.notna(row[field]) and row[field] != '')
...
print(f"\n=== FINAL STATISTICS ===")
print(f"ParentTaskID links: {parent_percentage_after:.2f}% ({parent_links_after} tasks)")
