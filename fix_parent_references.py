...
for initiative, tasks in initiative_groups.items():
    ...
    potential_parents = [t for t in tasks if pd.isna(t[1]['ParentTaskID']) or t[1]['ParentTaskID'] == '']
    
    child_candidates = [t for t in tasks if t[1]['TaskID'] not in df['ParentTaskID'].dropna().values]
    ...
    for p_idx, p_row in potential_parents:
        ...
        for c_idx, c_row in child_candidates:
            ...
            if (p_name in c_name or c_name in p_name) and p_name != c_name:
                new_relationships.append((p_id, c_row['TaskID'], c_idx, 'Name_Similarity'))
                
            elif pd.notna(p_row['TaskDescription']) and pd.notna(c_row['TaskDescription']):
                p_desc = normalize_text(p_row['TaskDescription'])
                c_desc = normalize_text(c_row['TaskDescription'])
                
                if (p_desc in c_desc or c_desc in p_desc) and len(p_desc) > 20 and len(c_desc) > 20:
                    new_relationships.append((p_id, c_row['TaskID'], c_idx, 'Description_Similarity'))
...
for group, tasks in transcription_groups.items():
    ...
    if not has_children:
        for c_idx, c_row in tasks[1:]:
            ...
            new_relationships.append((p_id, c_row['TaskID'], c_idx, 'Transcription_Group'))

if len(new_relationships) > additional_links_needed:
    transcription_relationships = [r for r in new_relationships if r[3] == 'Transcription_Group']
    name_relationships = [r for r in new_relationships if r[3] == 'Name_Similarity']
    desc_relationships = [r for r in new_relationships if r[3] == 'Description_Similarity']
    
    prioritized_relationships = transcription_relationships + name_relationships + desc_relationships
...
with open(report_path, 'w') as f:
    ...
    f.write(f"Found and fixed {len(invalid_parents)} tasks with invalid ParentTaskID references:\n\n")
