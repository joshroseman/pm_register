...
for field in key_fields:
    ...
    if field == "TaskName":
        ...
        df.loc[mask, "TaskDescription"] = df.loc[mask, "TaskName"].apply(
            lambda x: f"This task involves {x.lower()}."
        )
        changes += mask.sum()
        ...
        mask = df["ClassificationCategory"].isna() | (
            df["ClassificationCategory"] == ""
        )
        df.loc[mask, "ClassificationCategory"] = "unclassified"
        changes += mask.sum()

    elif field == "IsInformationalContext":
        mask = df["IsInformationalContext"].isna() | (
        ...
        mask = df["TranscriptionMatchNotes"].isna() | (
            df["TranscriptionMatchNotes"] == ""
        )
        df.loc[mask, "TranscriptionMatchNotes"] = (
            "No transcription match information available"
        )
        changes += mask.sum()
