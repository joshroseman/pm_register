...
def evaluate_description(desc):
    ...
    context_markers = [
        'this', 'specific', 'should', 'need', 'must', 'ensure', 'verify', 
        'implement', 'develop', 'create', 'update', 'modify', 'review'
    ]
    
    context_score = sum([1 for marker in context_markers if marker in normalized.split()])
...
with open(report_path, 'w') as f:
    ...
    f.write(f"Analysis of {len(similar_groups)} groups of similar tasks based on description content.\n\n")
    ...
    for group_idx, (key, tasks) in enumerate(sorted_groups):
        ...
        non_informative = [t for t in task_analyses if t['desc_type'] in ['empty', 'non_informative']]
        minimal = [t for t in task_analyses if t['desc_type'] == 'minimal']
        basic = [t for t in task_analyses if t['desc_type'] == 'basic']
        good_quality = [t for t in task_analyses if t['desc_type'] in ['contextual', 'detailed']]
        ...
        if non_informative:
            ...
            for task in non_informative:
                desc_preview = str(task['desc'])[:50] + "..." if len(str(task['desc'])) > 50 else task['desc']
                desc_preview = desc_preview.replace('|', '\\|').replace('\n', ' ')
                
                f.write(f"| {task['task_id']} | {first_task_name} | {desc_preview} | {task['desc_type']} |\n")
        ...
        if minimal and len(minimal) > 1:
            ...
            if delete:
                ...
                for task in delete:
                    ...
                    f.write(f"| {task['task_id']} | {first_task_name} | {desc_preview} | duplicate minimal description |\n")
                    tasks_to_delete.append(task)
        ...
        if good_quality:
            ...
            for task in good_quality:
                ...
                f.write(f"| {task['task_id']} | {first_task_name} | {new_name} | {desc_preview} |\n")
                ...
                if len(init_tasks) > 1:
                    ...
                    if delete:
                        ...
                        for task in delete:
                            ...
                            f.write(f"| {task['task_id']} | {first_task_name} | {desc_preview} | redundant basic description |\n")
                            tasks_to_delete.append(task)
    ...
    if tasks_to_rename:
        ...
        for task in tasks_to_rename:
            f.write(f"| {task['task_id']} | {task['old_name']} | {task['new_name']} |\n")
