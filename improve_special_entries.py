...
info_context_before = (df['IsInformationalContext'].notna() & (df['IsInformationalContext'] != '')).mean() * 100
...
mask = (
    df['ClassificationCategory'].isin(informational_categories) & 
...
df.loc[rasa_mask, 'ContextProcessedNotes'] = 'INFO_CTX: RASA-related entry identified and flagged.'

not_info_mask = (df['IsInformationalContext'].isna() | (df['IsInformationalContext'] == ''))
df.loc[not_info_mask, 'IsInformationalContext'] = 'False'

info_context_after = (df['IsInformationalContext'].notna() & (df['IsInformationalContext'] != '')).mean() * 100
