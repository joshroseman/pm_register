...
with open(report_path, 'w') as f:
    ...
    f.write(f"Found {len(similar_groups)} groups of tasks with potentially similar names.\n\n")
    
    sorted_groups = sorted(similar_groups.items(), key=lambda x: len(x[1]), reverse=True)
    ...
    for i, (key, tasks) in enumerate(sorted_groups):
        ...
        f.write("| TaskID | TaskName | TaskDescription | Assignee | Initiative | ParentTaskID |\n")
        f.write("|--------|----------|----------------|----------|------------|-------------|\n")
        ...
        for idx, task in tasks:
            ...
            f.write(f"| {task_id} | {task_name} | {description} | {assignee} | {initiative} | {parent_id} |\n")
        
        f.write("\n")

review_tasks_key = next((k for k, v in similar_groups.items() if 'review and clarify' in k), None)
if review_tasks_key:
    ...
    with open('review_tasks_analysis.md', 'w') as f:
        ...
        descriptions = [normalize_text(task[1]['TaskDescription']) for task in review_tasks]
        ...
        for idx, task in review_tasks:
            parent_id = task['ParentTaskID'] if pd.notna(task['ParentTaskID']) else 'None'
        ...
        for initiative, tasks in sorted(initiative_groups.items(), key=lambda x: len(x[1]), reverse=True):
            f.write(f"- {initiative}: {len(tasks)} tasks\n")
        
        f.write("\n### Recommendation:\n\n")
        if len(unique_descriptions) == 1:
            f.write("Since all these tasks have identical names and descriptions, they can be consolidated. ")
            f.write("Consider keeping one task per initiative and deleting the rest, or appending a [review/clarify] tag to parent tasks.\n\n")
        else:
            f.write("These tasks have identical names but different descriptions. ")
            f.write("Consider reviewing each description to determine if tasks can be consolidated by initiative or parent task.\n\n")

print(f"Generated report at {report_path}")
print(f"Generated special analysis for 'review and clarify' tasks at review_tasks_analysis.md")
