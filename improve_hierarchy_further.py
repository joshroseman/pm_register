...
tasks_with_parents = df[df['ParentTaskID'].notna() & (df['ParentTaskID'] != '')].shape[0]
...
for key, tasks in initiative_assignee_groups.items():
    ...
    if not has_children:
        for c_idx, c_row in tasks[1:]:
            ...
            p_desc = normalize_text(p_row['TaskDescription']) if pd.notna(p_row['TaskDescription']) else ''
            c_desc = normalize_text(c_row['TaskDescription']) if pd.notna(c_row['TaskDescription']) else ''
            
            if (p_name and c_name and (p_name in c_name or c_name in p_name or 
                                      any(word in c_name for word in p_name.split() if len(word) > 3))) or \
               (p_desc and c_desc and (p_desc in c_desc or c_desc in p_desc or 
                                      any(word in c_desc for word in p_desc.split() if len(word) > 3))):
                new_relationships.append((p_id, c_row['TaskID'], c_idx, 'Initiative_Assignee'))

keyword_groups = {}
important_keywords = [
    'studio', 'maintenance', 'retail', 'cafe', 'finance', 'admin', 'task', 'management',
    'system', 'integration', 'development', 'facility', 'organizational', 'meta', 'coordination',
    'rasa', 'field', 'memory', 'weatherstone', 'rhythm', 'modeling', 'export', 'import',
    'booking', 'marketing', 'business', 'program', 'symbolic', 'structure'
]
...
if len(new_relationships) > additional_links_needed:
    ...
    initiative_assignee_relationships = [r for r in new_relationships if r[3] == 'Initiative_Assignee']
    keyword_relationships = [r for r in new_relationships if r[3] == 'Keyword']
    
    prioritized_relationships = name_prefix_relationships + initiative_assignee_relationships + keyword_relationships
...
with open(report_path, 'w') as f:
    ...
    f.write(f"Initial ParentTaskID links: {tasks_with_parents - relationships_added} ({(tasks_with_parents - relationships_added) / len(df) * 100:.2f}%)\n\n")
    ...
    if new_relationships:
        ...
        for parent_id, child_id, _, method in new_relationships[:20]:  # Show first 20 for brevity
    ...
    f.write("2. **Initiative_Assignee**: Tasks with the same initiative and assignee, and some similarity in names or descriptions\n")
    f.write("3. **Keyword**: Tasks with similar important keywords in their descriptions\n")
