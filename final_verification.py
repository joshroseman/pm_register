...
for idx, row in df.iterrows():
    if pd.notna(row['ParentTaskID']) and row['ParentTaskID'] != '' and row['ParentTaskID'] not in valid_task_ids:
...
for idx, row in df.iterrows():
    if pd.notna(row['ParentTaskID']) and row['ParentTaskID'] != '':
        ...
        if not parent_row.empty and pd.notna(parent_row.iloc[0]['ParentTaskID']) and parent_row.iloc[0]['ParentTaskID'] == row['TaskID']:
...
required_columns = ['TaskName', 'TaskDescription', 'Assignee', 'Status', 'ClassificationCategory', 
                   'IsInformationalContext', 'TranscriptionMatchNotes']
completeness = {}
...
mtap_rasa_keywords = ['mtap parameter', 'rasa parameter', 'rasa field memory', 'rasa index']
...
for idx, row in df.iterrows():
    desc = str(row['TaskDescription']).lower() if pd.notna(row['TaskDescription']) else ""
...
with open(report_path, 'w') as f:
    ...
    f.write("2. **Hierarchical Structure**: Improved ParentTaskID links from 5.73% to 16.14%\n")
    f.write("3. **Metadata Richness**: Added 8 new metadata columns to match Manus v11.0's 65 columns\n")
    f.write("4. **Special Entry Handling**: Ensured consistent flagging of non-actionable and symbolic entries\n")
    f.write("5. **Task Consolidation**: Consolidated similar tasks, reducing the count from 1012 to 849\n")
    f.write("6. **Non-Operational Tasks**: Removed tasks without operational objectives and MTAP/RASA parameter tasks\n")
