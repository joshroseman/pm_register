...
if "ParentTaskID" in df.columns:
    ...
    print(
        f"Tasks with parent links: {len(parent_links)} ({len(parent_links)/len(df)*100:.2f}%)"
    )
    ...
    if len(invalid_parents) > 0:
        print(
            f"WARNING: Found {len(invalid_parents)} tasks with invalid ParentTaskID references"
        )
        print("Sample of invalid references:")
        for idx, row in invalid_parents.head(5).iterrows():
            print(
                f"  Task {row['TaskID']} references non-existent parent {row['ParentTaskID']}"
            )
    else:
    ...
    for idx, row in parent_links.iterrows():
        ...
        for _ in range(5):  # Limit depth to avoid infinite loops
            ...
            if (
                parent_row.empty
                or pd.isna(parent_row.iloc[0]["ParentTaskID"])
                or parent_row.iloc[0]["ParentTaskID"] == ""
            ):
                break
    ...
    if "SubTaskLevel" in df.columns:
        ...
        if len(inconsistent_levels) > 0:
            print(
                f"WARNING: Found {len(inconsistent_levels)} tasks with parents but SubTaskLevel = 0"
            )

        no_parent_tasks = df[df["ParentTaskID"].isna() | (df["ParentTaskID"] == "")]
        no_parent_tasks_numeric = no_parent_tasks.copy()
        no_parent_tasks_numeric["SubTaskLevel"] = pd.to_numeric(
            no_parent_tasks_numeric["SubTaskLevel"], errors="coerce"
        )
        inconsistent_no_parent = no_parent_tasks_numeric[
            no_parent_tasks_numeric["SubTaskLevel"] > 0
        ]
        if len(inconsistent_no_parent) > 0:
            print(
                f"WARNING: Found {len(inconsistent_no_parent)} tasks without parents but SubTaskLevel > 0"
            )
    ...
    def get_depth(task_id, visited=None):
        ...
        if task_id not in hierarchy_map:
            return 1
        return 1 + max(
            [get_depth(child, visited.copy()) for child in hierarchy_map[task_id]],
            default=0,
        )
        ...
        if len(children) > 1:
            ...
            if not parent_row.empty:
                ...
                for child_id in children:
                    ...
                    if (
                        not child_row.empty
                        and child_row.iloc[0]["Initiative"] != parent_initiative
                    ):
                        initiative_coherence_issues += 1
                        if initiative_coherence_issues <= 5:  # Limit examples
                            print(
                                f"  Initiative mismatch: Parent {parent_id} ({parent_initiative}) has child {child_id} ({child_row.iloc[0]['Initiative']})"
                            )

    if initiative_coherence_issues > 0:
        print(
            f"WARNING: Found {initiative_coherence_issues} tasks with initiative mismatches between parent and child"
        )
    else:
...
if "IsInformationalContext" in df.columns and "ClassificationCategory" in df.columns:
    ...
    print(
        f"Informational context tasks: {len(info_context_tasks)} ({len(info_context_tasks)/len(df)*100:.2f}%)"
    )
    ...
    if len(rasa_not_info) > 0:
        print(
            f"WARNING: Found {len(rasa_not_info)} tasks with RASA references not marked as informational contexts"
        )
    else:
        print(
            "All tasks with RASA references are properly marked as informational contexts"
        )
...
if potential_duplicates:
    print(
        f"Found {len(potential_duplicates)} groups of tasks with potentially similar names"
    )
...
if "TranscriptionGroup" in df.columns:
    ...
    print(
        f"  Tasks without group: {transcription_groups.get('', 0) + transcription_groups.get(np.nan, 0)}"
    )
    print(
        f"  Largest group size: {transcription_groups.iloc[0] if not transcription_groups.empty else 0}"
    )
    print(
        f"  Groups with only one task: {sum(1 for count in transcription_groups if count == 1)}"
    )
...
print(
    f"   - Current ParentTaskID links: {len(parent_links)/len(df)*100:.2f}% (target: 16.22%)"
)
print(f"   - SubTaskLevel population: 100%")
print(
    "   - Recommendation: Further improve hierarchical relationships to reach target of 16.22%"
)
...
if "invalid_parents" in locals() and len(invalid_parents) > 0:
    print(
        f"   - WARNING: {len(invalid_parents)} invalid parent references need to be fixed"
    )
if "initiative_coherence_issues" in locals() and initiative_coherence_issues > 0:
    print(
        f"   - WARNING: {initiative_coherence_issues} initiative mismatches between parents and children"
    )

print("\n3. Special Entry Handling:")
if "rasa_not_info" in locals() and len(rasa_not_info) > 0:
    print(
        f"   - WARNING: {len(rasa_not_info)} RASA references not marked as informational contexts"
    )
else:
...
if potential_duplicates:
    print(
        f"   - {len(potential_duplicates)} groups of potentially similar tasks should be reviewed for consolidation"
    )
else:
